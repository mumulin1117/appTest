...
import UIKit
import SnapKit
class DSORDiscoverRecentCel: UICollectionViewCell {
    ...
    override init(frame: CGRect) {
        ...
        let voberDSOR = UIImageView(image: UIImage.init(named: "discove_topbg"))
        contentView.backgroundColor = .clear
        voberDSOR.contentMode = .scaleToFill
        contentView.addSubview(voberDSOR)
        voberDSOR.snp.makeConstraints { make in
            make.edges.equalToSuperview()
        }
        
        let berVfhuDSOR = UIImageView(image: UIImage.init(named: "homerheader_boee"))
        berVfhuDSOR.contentMode = .scaleToFill
        contentView.addSubview(berVfhuDSOR)
        ...
        robertnameDSOR.textColor = UIColor(red: 0.16, green: 0.02, blue: 0.02, alpha: 1)
        ...
        contentView.addSubview(robercentHEaderDSOR)
        robercentHEaderDSOR.snp.makeConstraints { make in
            make.width.height.equalTo(42)
            make.center.equalTo(berVfhuDSOR)
        }
        
        contentView.addSubview(robertnameDSOR)
        robertnameDSOR.snp.makeConstraints { make in
            make.centerX.equalToSuperview()
            make.top.equalTo(berVfhuDSOR.snp.bottom).offset(8)
