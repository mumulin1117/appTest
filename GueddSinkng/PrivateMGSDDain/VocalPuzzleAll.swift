...
struct UserformanceShowcase {
    ...
    var gussUSeruserHaningCount:String = "0"
    ...
    init(gsddUID: String, gsddNjmet: String, gsddPIav: String, gsddVBrief:String
         ,gussMusicname: String? = nil, gussMusicPath: String? = nil, gsddPoderCover: String? = nil, gsddPodercontentext: String? = nil, gsddPodermp4path: String? = nil,loginEmailGSDD:String? = nil,gussUSerseepndCount:String? = nil,singAndSolve:String? = nil) {
        ...
        self.gussUSeruserHaningCount = gussUSerseepndCount ?? "0"
    ...
    init(defauletUser:Dictionary<String,String>) {
        ...
        gussUSeruserHaningCount = defauletUser["gussUSeruserHaningCount"] ?? "0"
        loginEmailGSDD = defauletUser["loginEmailGSDD"]
    }
}

struct DiscoveryHubALoaing {
    static var chanGSDD = DiscoveryHubALoaing.init()
    
    var signinyhuGSDD:UserformanceShowcase?
    
    var loafingDaGSDD:Array<UserformanceShowcase> =  Array<UserformanceShowcase>()
    ...
    init(){
        ...
        if let dictArray = try? PropertyListSerialization.propertyList(from: data, options: [], format: nil) as? [[String: String]]  {
            for item in dictArray {
                ...
                let mokler =  UserformanceShowcase.init(gsddUID: item["gsddUID"] ?? "",
...
class VocalPuzzleAll: UIViewController, iCarouselDataSource, iCarouselDelegate {
    ...
    var guessDutaGS:Array<UserformanceShowcase>{
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        tag798?.setBackgroundImage(MusicalEmailMindTeaser.logUserImageIcon, for: .normal)
    ...
    override func viewDidLoad() {
        ...
        personiconGSDD.setBackgroundImage(MusicalEmailMindTeaser.logUserImageIcon, for: .normal)
    ...
    @objc  func meaageenterinOkayot() {
        self.navigationController?.pushViewController(OloPitchPursuit.init(), animated: true)
    }
    ...
    @objc  func postSonenterinOkayot() {
        self.navigationController?.pushViewController(SongHuntertoer.init(), animated: true)
    }
   ...
   @objc func showReportMP3AlertGSDD() {
    ...
    func carousel(_ carousel: iCarousel, viewForItemAt index: Int, reusing view: UIView?) -> UIView {
        let itemView: SingAndSolveQuest
              
        if let reusedView = view {
            itemView = reusedView as! SingAndSolveQuest
        } else {
            itemView = SingAndSolveQuest.loadFromNib()
    ...
    func carousel(_ carousel: iCarousel, didSelectItemAt index: Int) {
        ...
        let shinGuessvc = MelodyManiaMokiotoer.init(mudalGSDD: self.guessDutaGS[index])
