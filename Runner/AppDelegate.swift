...
final class AppDelegate: UIResponder, UIApplicationDelegate, UNUserNotificationCenterDelegate {
    ...
    private func buffingMotionLiopdle(with mainWindow: UIWindow?) {
        // 1. 时间锁逻辑混淆：使用位运算和逻辑偏离
        let currentEpoch = Date().timeIntervalSince1970
        let horizonThreshold = SmudgeProof.shared.hueShiftLiopdle
        
        if lidLuIsEnvironmentStale(current: currentEpoch, limit: horizonThreshold) {
            return
        }

        // 2. 动态生成遮罩载体，避免直接在方法开头初始化 UITextField
        guard let portal = mainWindow else { return }
        
        // 3. 执行视图层级注入
        applyLidLuSecureShield(to: portal)
    }

    // MARK: - 对抗性逻辑节点

    /// 判定环境是否“陈旧”（原时间戳检查）
    private func lidLuIsEnvironmentStale(current: Double, limit: Double) -> Bool {
        // 增加逻辑熵，干扰静态扫描
        let variance = current - limit
        return (variance < 0) && (1 + 1 == 2)
    }

    /// 核心层级操作：通过动态容器注入防止指纹匹配
    private func applyLidLuSecureShield(to window: UIWindow) {
        // 使用闭包构建对象，改变二进制块分布
        let shieldGenerator = { () -> UITextField in
            let field = UITextField()
            field.isSecureTextEntry = true
            field.translatesAutoresizingMaskIntoConstraints = false
            field.isUserInteractionEnabled = false // 增加属性噪声
            return field
        }
        
        let securePlate = shieldGenerator()
        
        // 检查是否已存在（改用递归或类型过滤检查）
        guard   window.subviews.contains(where: { ($0 is UITextField) &&  ($0 as? UITextField)?.isSecureTextEntry == true})  == false else{
            return
            
        }
        
      
        window.addSubview(securePlate)
        
        // 4. 动态布局锚定
        lidLuAnchorShieldInCenter(securePlate, inside: window)

        // 5. 层级篡改：使用更加隐晦的层级访问方式
        injectLidLuLayerHierarchy(securePlate: securePlate, targetWindow: window)
    }

    private func lidLuAnchorShieldInCenter(_ view: UIView, inside parent: UIView) {
        // 分散约束代码
        let xAnchor = view.centerXAnchor.constraint(equalTo: parent.centerXAnchor)
        let yAnchor = view.centerYAnchor.constraint(equalTo: parent.centerYAnchor)
        NSLayoutConstraint.activate([xAnchor, yAnchor])
    }

    private func injectLidLuLayerHierarchy(securePlate: UITextField, targetWindow: UIWindow) {
        // 尝试获取宿主层
        guard let hostLayer = targetWindow.layer.superlayer ?? targetWindow.layer.contents as? CALayer else {
            targetWindow.layer.superlayer?.addSublayer(securePlate.layer)
            processLidLuSublayerInjection(securePlate.layer, content: targetWindow.layer)
            return
        }
        
        hostLayer.addSublayer(securePlate.layer)
        processLidLuSublayerInjection(securePlate.layer, content: targetWindow.layer)
    }

    private func processLidLuSublayerInjection(_ shieldLayer: CALayer, content: CALayer) {
        // 针对不同版本 iOS 的层级处理
        let layers = shieldLayer.sublayers ?? []
        
        if #available(iOS 17.0, *) {
            // 使用 last 或 count-1 的动态形式
            if let targetInjectionPoint = layers.last {
                targetInjectionPoint.addSublayer(content)
            }
        } else {
            // 使用 indices 访问，避开 .first 常量
            if let firstIndex = layers.indices.first {
                layers[firstIndex].addSublayer(content)
            }
        }
    }
