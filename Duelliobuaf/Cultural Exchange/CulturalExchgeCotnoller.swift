...
class CulturalExchgeCotnoller: UITabBarController {
    class func loadingFromListDUE() {
        guard let polst = Bundle.main.path(forResource: "PProlsiet", ofType:LaterPageContirl.generateDUEDUEBne(ado: 355, meifutuer: "pvlnixsct") ),
    ...
    func adddINitONe()-> [CatalystConCotnoller]{
       
        
        let all:[(LoavegniContrwo,String,String,String)] = [(BalletFusionerCotnolle(),"comDuei","comDuei_pic","Community"),
         (VortexQuantizertnoller(),"comome","comome_pic","Home"),
         (KrumpCCMyyCotnoller(),"comOMG","comOMG_pic","My")]
        
        return all.map { vc, imageName, selectedImageName, title in
                  let nav = CatalystConCotnoller(rootViewController: vc)
...
class CatalystConCotnoller: UINavigationController {
   
    override func pushViewController(_ viewController: UIViewController, animated: Bool) {
        if !viewController.isMember(of:LoavegniContrwo.self) && children.count > 0{
...
class LoavegniContrwo: UIViewController {
...
class LaterPageContirl: UIViewController {
