...
class WeeklyThemedtnteller: HiRoHRalFllaterPicdert, ShareableGistntellerGibeDele {
    ...
    func AddHIRDIInitelayer()  {
        ...
        guard let videlinkg = igjiii["hiroCamera_videws"]?.components(separatedBy: "@@@").first ,
           let FanaaTariqa = Bundle.main.path(forResource: videlinkg, ofType: "mp4") else{
           return
        }
       
        
        let Hirdishname = URL(fileURLWithPath: FanaaTariqa)
   ...
   @objc func Vafgkuo() {
    ...
    private func startGestureChallenge() {
        ...
        view.addSubview(SamhitaJnanaVijnana)
        view.addSubview(dancePickBuuteon)
                motionMantraChakra.layer.cornerRadius = 15
                motionMantraChakra.layer.masksToBounds = true
        view.addSubview(        motionMantraChakra)
        ...
        SamhitaJnanaVijnana.snp.makeConstraints { make in
            make.width.equalTo(74)
            make.height.equalTo(54)
        ...
        GiaftBuuteon.snp.makeConstraints { make in
            ...
            make.centerY.equalTo(SamhitaJnanaVijnana)
            make.centerX.equalToSuperview()
        }
        ...
        dancePickBuuteon.snp.makeConstraints { make in
            ...
            make.bottom.equalTo(SamhitaJnanaVijnana.snp.top).offset(-30)
        }
        
                motionMantraChakra.snp.makeConstraints { make in
            make.width.height.equalTo(70)
    ...
    private func connectWithDancerProfile() {
        ...
        if let videlinkg = igjiii["hiroCamera_videws"]?.components(separatedBy: "@@@").first{
            posphereneHo.image = StaccatoSamarasaYoga.chacheImage[videlinkg]
        }
    }
    
   
    private lazy var         motionMantraChakra: UIButton = {
        let ad = UIButton.init()
     let dkf =  ["Vorric_pic","Ysari_pic","Xypher","Rhysan","Luxen","Taelith","Brenza","Ozym"]
        ...
        ad.setImage(UIImage.init(named: "iconPoikio_play"), for: .normal)
        ad.addTarget(self, action: #selector(tantyPickDance), for: .touchUpInside)
        return ad
    }()
    //
    @objc func tantyPickDance() {
...
//        perngs.modalPresentationStyle = .overCurrentContext
   ...
   @objc func centeruserPuple() {
    ...
    private lazy var SamhitaJnanaVijnana: UIButton = {
        let ad = UIButton.init()
        ad.setImage(UIImage.init(named: "Jnana"), for: .normal)
        ad.addTarget(self, action: #selector(gestureGuruGita), for: .touchUpInside)
        return ad
    }()
    //聊天
   @objc func gestureGuruGita() {
       guard  let Guru = ConVerSationLisrMan.shared.conversations.filter { harmonic in
           return harmonic.diploagUser == self.igjiii
       }.first else{
           return
       }
       let Converds = PoseParabrahmanNritya.init(shouleinger: Guru)
   ...
   @objc func showGIFTuserPuple() {
       let fokbo = ShareableGistnteller.init()
       
       let presentVc = Focusesintersection.init(rootViewController:fokbo )
