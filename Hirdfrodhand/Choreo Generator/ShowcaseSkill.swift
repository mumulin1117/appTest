...
class ShowcaseSkill: HolePicdwei {
    ...
    @objc func traikerSetoaskkdo()  {
        self.navigationController?.pushViewController(ARIntegration.init(), animated: true)
    }
    //edit
    ...
    @objc func traikerEdittoaskkdo()  {
        self.navigationController?.pushViewController(Creativeexpressioner.init(), animated: true)
    }
   ...
   @objc func toblancelgfor()  {
    ...
    @objc func oaskkdo()  {
        if let sudh = UserDefaults.standard.object(forKey: "logeduserhiedIndj") as? Dictionary<String,String>{
            self.navigationController?.pushViewController(HikmatGuruifat.init(igjiii: sudh, ismecenet: true), animated: true)
        }
