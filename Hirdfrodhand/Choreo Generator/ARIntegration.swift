...
import UIKit
//设置 
class ARIntegration: HiRoHRalFllaterPicdert {
    ...
    @objc func cdfterINgtag(dkxkd:UIButton) {
        ...
        if taginsepick == 0 {
            self.navigationController?.pushViewController(SmulationRectooner.init(tttttt: 1), animated: true)
        }
        
        if taginsepick == 1 {
            self.navigationController?.pushViewController(SmulationRectooner.init(tttttt: 2), animated: true)
        }
