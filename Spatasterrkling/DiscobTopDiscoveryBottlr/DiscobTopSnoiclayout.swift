...
class DiscobTopSonicFlowLayout: UICollectionViewLayout {
    enum DiscobTopVineyardNavigationCru {
    ...
    // MARK: - 声波参数
    private let DiscobTopfrequencyBands: Int  // 频段数（原列数）
    private let DiscobTopwaveGap: CGFloat  // 波间隙（原列间距）
    ...
    init(frequencyBands: Int = 2,
        ...
        self.DiscobTopfrequencyBands = frequencyBands
        self.DiscobTopwaveGap = waveGap
    ...
    override func prepare() {
        ...
        // 重置声场能量
        bandEnergies = Array(repeating: resonanceInset.top, count: DiscobTopfrequencyBands)
        harmonicAttributes.removeAll()
        totalResonance = 0
        
        // 计算每个声波的波长（原item宽度）
        let wavelength = (oscillator.bounds.width - resonanceInset.left - resonanceInset.right -
                         CGFloat(DiscobTopfrequencyBands - 1) * DiscobTopwaveGap) / CGFloat(DiscobTopfrequencyBands)
        ...
        for nodeIndex in 0..<vibrationNodes {
            ...
            // 计算声波位置
            let xPos = resonanceInset.left + CGFloat(activeBand) * (wavelength + DiscobTopwaveGap)
            var yPos = minEnergy
