//
//  JPIDPAGHomeController.swift
//  JusperXdweSpein
//
//  Created by mumu on 2025/11/20.
//

import UIKit


class JPIDPAGHomeController: JPIDPAGSXPRIcyousg, UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if collectionView == self.topuserNarrativeNFTs {
            ...
            if let imglink = ploraUserData[indexPath.row]["microNarrative"] as? String,let uri = URL(string: imglink) {
                plora.usertBiag.JPIDPAGlaodImage(JPIDPAGfrom: uri)
            }
           
            return plora
        }
        
        let plora = collectionView.dequeueReusableCell(withReuseIdentifier: "JPIDPAGJPIDMomentCell", for: indexPath) as! JPIDPAGJPIDMomentCell
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if collectionView == self.topuserNarrativeNFTs {
            ...
            //用户个人中心
            let linkader = JPIDPAGSXPRequpour.shared.appBaseUrlAVoutWEB + self.captionic(storymorph:"pyajgtelst/qHwozmgenPhaugrep/eidngdwedxf?tutsoebriIpdb=") + "\(userid)"
       ...
       // aiIntentParser    storyEncrypt aiIntentParser narrativeNFTs aestheticAds
        ...
        let linkader = JPIDPAGSXPRequpour.shared.appBaseUrlAVoutWEB + self.captionic(storymorph:"pgahgmemsr/oDnyvncahmhiycgDuevtgaoiflosp/litnsdhedxd?odqyknkagmtidctIkdm=") + "\(itemid)"
        reactionDensity(linkader:linkader)
    ...
    @objc func aestheticRespect()  {
        ...
        reactionDensity(linkader:JPIDPAGSXPRequpour.shared.appBaseUrlAVoutWEB + self.captionic(storymorph:"ppangmeesc/rRzespjowrptt/yiyngdfesxc?"))
     }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        let userparameters: [String:Any] = [
            "plogging": JPIDPAGSXPRequpour.shared.appQuicklyId //bundleId
        ]
        plogView.isUserInteractionEnabled = false
        plogView.tag = 44
        JPIDPAGSXPRequpour.shared.makeRequest(path: "/gazogmfndmeicz/pqrytzaoqihd",parameters: userparameters,includeLoading:true) { responses in
    ...
    func FrameRewards()  {
        ...
        momentMicroLicensesView.register(JPIDPAGJPIDMomentCell.self, forCellWithReuseIdentifier: "JPIDPAGJPIDMomentCell")
    ...
    @objc func VisualGrants() {
        reactionDensity(linkader:JPIDPAGSXPRequpour.shared.appBaseUrlAVoutWEB + self.captionic(storymorph:"pzavgjexsr/tibnjfiotramiavtoicojnv/hiwnpdgelxw?"))
