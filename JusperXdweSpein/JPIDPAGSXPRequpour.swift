...
struct PloraStoryContent {
    let mediaType: JPIDPAGPloraMediaType
    let assets: [URL]?
    let caption: String?
  
}
class JPIDPAGSXPRequpour {
    ...
    static let shared = JPIDPAGSXPRequpour()
    ...
    func makeRequest(
        ...
        if includeLoading {
            ...
            finalHeaders[self.captionic(storymorph: "tkowkweqn")] = JPIDPAGTrrendTides?["echozoa"] as? String
        }
